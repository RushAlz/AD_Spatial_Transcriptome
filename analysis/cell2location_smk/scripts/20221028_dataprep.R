...
split_by_section_folder = "/pastel/projects/spatial_t/cell2location/data/20221006_expressionData/split_by_section_fix"
...
write_tsv(df_old, file = paste0(smk_folder, "/avgNuclei_perSample.old.txt"))

# Manual FIX from Denis
df[paste0(df$sample,"_",df$section) == "B18007_3", "avg_nuclei"] = 3.428571429
df[paste0(df$sample,"_",df$section) == "B17085_4", "avg_nuclei"] = 3.571428571
df[paste0(df$sample,"_",df$section) == "B16100_4", "avg_nuclei"] = 4.321428571
df[paste0(df$sample,"_",df$section) == "B17078_3", "avg_nuclei"] = 4.214285714
df[paste0(df$sample,"_",df$section) == "B16037_1", "avg_nuclei"] = 4.714285714
df[paste0(df$sample,"_",df$section) == "B16037_4", "avg_nuclei"] = 4.642857143
df[paste0(df$sample,"_",df$section) == "B17028_4", "avg_nuclei"] = 4.678571429
df[paste0(df$sample,"_",df$section) == "B17078_1", "avg_nuclei"] = 4.892857143
df[paste0(df$sample,"_",df$section) == "B16080_4", "avg_nuclei"] = 4.75
#df[paste0(df$sample,"_",df$section) == "B16006_1", "avg_nuclei"] # = 3.642857143 # VistoSeq below 10
#df[paste0(df$sample,"_",df$section) == "B16006_2", "avg_nuclei"] # = 3.857142857 # VistoSeq below 10
df[paste0(df$sample,"_",df$section) == "B17093_4", "avg_nuclei"] = NA # discard
df = na.omit(df)
# Missing in the original nuclei counts table. Adding manually
df = rbind(df, data.frame(sample = "B16002", section = "3", avg_nuclei = 4.428571429))
df = rbind(df, data.frame(sample = "B16100", section = "2", avg_nuclei = 4.428571429))

df[order(df$avg_nuclei),]
# The expected avg nuclei counts should be around 5-10.

df_filt = df # 77
df_filt = df[df$avg_nuclei < 15,] # 77 sections with avg nuclei < 15
write_tsv(df_filt, file = paste0(smk_folder, "/avgNuclei_perSample_fix.txt"))
####
...
metadata_st$section = metadata_st$sample
metadata_st$section[metadata_st$section=="A1"] <- 1
metadata_st$section[metadata_st$section=="B1"] <- 2
metadata_st$section[metadata_st$section=="C1"] <- 3
metadata_st$section[metadata_st$section=="D1"] <- 4
dim(metadata_st[paste0(metadata_st$Bnum,"_",metadata_st$section) == "B16002_3", ]) 
dim(metadata_st[paste0(metadata_st$Bnum,"_",metadata_st$section) == "B16100_2", ]) 
...
# 77 of 78 files should be there
####
